...
public class CareFragment extends Fragment implements ViewPager.OnPageChangeListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    /*@ViewById(R.id.care_img_pager)
    AutoScrollViewPager mAutoScrollViewPager;
    @ViewById(R.id.care_img_text)
    TextView mTitleTextView;
    @ViewById(R.id.care_img_index)
...
//        mBannerList = new ArrayList<Article>();
        ...
        mCareAdapter = new TuringAdapter(mActivity, mArticleList);
    }
    ...
    void init() {
        ...
        mListView.setAdapter(mCareAdapter);
        mListView.setOnItemLoadClickListener(new LoadMoreView.OnItemLoadClickListener() {
            ...
            public void setOnItemLoadClick(int position, Object obj, View item) {
                Article a = mCareAdapter.getItem(position);
    ...
    /*private void requestBannerList() {
        //请求幻灯片
        mActivity.requestBanner(2, new RequestListener() {
            @Override
